...
from unet_resnet_copy import UNetWithResnet50Encoder
...
log_name = 'U-Net-resnet-finetunefinalconv_pred'
...
def train_fn(loader, model, optimizer, loss_fn,loss_fn2, scaler):
    ...
    for batch_idx, (data, targets) in enumerate(loop):
        ...
        with torch.cuda.amp.autocast():
            ...
            loss2 = loss_fn2(predictions, targets)
...
def main():
    ...
    model = UNetWithResnet50Encoder().to(DEVICE)  #to函数，可以将tensor变量拷贝到gpu，多个gpu还可以写多线程。
    ...
    def loss_fn2(y_pred, y_true):
        ...
        score = (2.0 * intersection + smooth) / (y_pred.sum() + y_true.sum() + smooth)
    ...
    class DnCNN(nn.Module):
        def __init__(self, in_channels=1,num_layers=17, num_filters=64):
            ...
            for _ in range(num_layers - 2):
                self.dncnn.append(nn.Conv2d(num_filters, num_filters, kernel_size, padding=padding, bias=False))
                self.dncnn.append(nn.BatchNorm2d(num_filters, eps=1e-4, momentum=0.99))
                self.dncnn.append(nn.ReLU(inplace=True))
            self.dncnn.append(nn.Conv2d(num_filters, in_channels,kernel_size = 3, stride=1, padding=1, bias=False))
    ...
    model.final_conv = DnCNN(num_layers=8,num_filters=64)
    ...
    if CHECK_ACC:
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            train_fn(train_loader, model, optimizer, loss_fn,loss_fn2, scaler)
            # 验证训练集
            model.eval()
            total_test_loss = 0
            with torch.no_grad():
                for batch_idx, (data, targets) in enumerate(train_loader):
                    ...
                    loss2 = loss_fn2(preds,targets)
                    total_test_loss = total_test_loss + loss.item()+ 0.25 *loss2.item()
